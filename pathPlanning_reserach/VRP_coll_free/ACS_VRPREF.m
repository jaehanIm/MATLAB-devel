...
for t = 1 : maxIter
    ...
    colony = [];
    colony = createColonyVRPREF( mapGraph, colony , antNo, tau, eta, param, vnum);
    
    
    % Calculate the fitness values of all ants 
    for i = 1 : antNo 
        [colony.ant(i).fitness, colony.ant(i).fitnessL, colony.ant(i).fitnessPer, colony.ant(i).fitnessM] = fitnessFunctionVRPREF(colony.ant(i).tour, colony.ant(i).vehTourLen, mapGraph, vnum);       
    end
    ...
    tau = updatePhromoneVRPREF(tau , colony, param.rho, minIndex, vnum);  
