...
totalVel = mean(totalVelocity(imuJobStartIdx(1):imuJobEndIdx(end)));
...
%% Job-mission group analysis
jobLen = length(photoJobCategory);
partialPhotoFault =[];
for i = 1:jobLen
    jobPhotoLen = length(photoJobCategory{i});
    partialPhotoFault(i) = sum(ismember(tripleFault,photoJobCategory{i}))/jobPhotoLen*100;
end
totalPhotoFault = length(tripleFault)/length(photoTime)*100;
...
% savetemp = [totalVel,totalSatis, totalPhotoScore];
savetemp = {};
savetemp.totalVel = totalVel;
savetemp.totalSatis = totalSatis;
savetemp.totalPhotoScore = totalPhotoScore;
save('1sortie_mission_fault.mat','savetemp')