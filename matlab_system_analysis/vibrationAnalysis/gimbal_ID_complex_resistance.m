...
fun = @(x) sum(((Y - lsim(feedback(...
    tf([x(1)*x(2) x(1)*x(3) x(1)*x(4)],[1 0])*...
    (tf(0.85,[1 2*x(5)*x(6) x(6)^2])-tf(x(7),[1 0 0]))*...
    tf(1,[1 0 0]),1),U,T)).^2));
...
[result,score] = fmincon(fun,[prelog(1) prelog(2) prelog(3) prelog(4) 1 100 0.1],[],[],[],[],[-inf -inf -inf -inf -inf -inf 0],[],[],options);
...
grid on
x = result
openTf = tf([x(1)*x(2) x(1)*x(3) x(1)*x(4)],[1 0])*...
    (tf(0.85,[1 2*x(5)*x(6) x(6)^2])-tf(x(7),[1 0 0]))*...
    tf(1,[1 0 0]);
